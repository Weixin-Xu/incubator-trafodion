...
unsigned long CDescRec::setDescRec(short DescMode, SQLItemDesc_def *SQLItemDesc)
{
		...
		if (m_ODBCCharset == SQLCHARSETCODE_UCS2)
	{
		...
		{
		case SQL_CHAR:
			if(m_DescDesc->m_ICUConv->getUCS2Translation())
				m_ODBCDataType = SQL_WCHAR;
			else
				m_ODBCDataType = SQL_CHAR;
			break;
		case SQL_VARCHAR:
	...
	// ALM CR5228: It was simply returning "NCHAR"/"NVARCHAR" for SQL_DESC_TYPE_NAME.
	// Although this does not disobey SQL&ODBC standard, Seaquest does not expose column type
	// name like this through any end user interface or tool, instead it completely shows
	// with "Character Set UCS2". Thus the type name descriptor now returns the describing
	// text with these information as well.
	if (m_ODBCCharset == SQLCHARSETCODE_UCS2)
	{
		switch(m_ODBCDataType)
		{
		case SQL_WCHAR:
			sprintf((char *)m_DescTypeName, "CHAR (%d) CHARACTER SET %s", SQLItemDesc->maxLen/2, SQLCHARSETSTRING_UCS2);
			break;
		case SQL_WVARCHAR:
			sprintf((char *)m_DescTypeName, "VARCHAR (%d) CHARACTER SET %s", SQLItemDesc->maxLen/2, SQLCHARSETSTRING_UCS2);
			break;
		default:
			strcpy((char *)m_DescTypeName, gSQLDatatypeMap[i].typeName);
			break;
	...
	else if (m_ODBCCharset == SQLCHARSETCODE_UTF8)
	{
		switch(m_ODBCDataType)
		{
		case SQL_CHAR:
		case SQL_WCHAR:
			sprintf((char *)m_DescTypeName, "CHAR (%d) CHARACTER SET %s", SQLItemDesc->maxLen/4, SQLCHARSETSTRING_UTF8);
			break;
		case SQL_VARCHAR:
		case SQL_WVARCHAR:
			sprintf((char *)m_DescTypeName, "VARCHAR (%d) CHARACTER SET %s", SQLItemDesc->maxLen/4, SQLCHARSETSTRING_UTF8);
			break;
		default:
			strcpy((char *)m_DescTypeName, gSQLDatatypeMap[i].typeName);
			break;
	...
	case SQL_LONGVARCHAR:
		...
		case SQLCHARSETCODE_UTF8:
			sprintf((char *)m_DescLocalTypeName,"%s CHARACTER SET %s", gSQLDatatypeMap[i].typeName, SQLCHARSETSTRING_UTF8);
			break;
		default:
...
//#if defined(_WIN64) || defined(__LP64__)
	...
	long          LongValuePtr  = (long)ValuePtr;
//#endif

	value = (SQLSMALLINT)((SQLINTEGER)LongValuePtr);
	...
	case SQL_DESC_CONCISE_TYPE:
		switch (DescMode)
		{
			...
			case SQL_ATTR_IMP_PARAM_DESC:
				for (i = 0, found = FALSE; gSQLDatatypeMap[i].conciseType != SQL_DEFAULT ; i++)
				{
					if (gSQLDatatypeMap[i].conciseType == value)
					{
						found = TRUE;
			...
			case SQL_ATTR_APP_PARAM_DESC:
			default:
				if (value != SQL_C_DEFAULT)
				{
					for (i = 0, found = FALSE; gCDatatypeMap[i].conciseType != SQL_C_DEFAULT ; i++)
					{
						if (gCDatatypeMap[i].conciseType == value)
						{
							found = TRUE;
	...
	case SQL_DESC_TYPE:
		switch (DescMode)
		{
			...
			case SQL_ATTR_IMP_PARAM_DESC:
				for (i = 0, found = FALSE; gSQLDatatypeMap[i].conciseType != SQL_DEFAULT ; i++)
				{
					if (gSQLDatatypeMap[i].verboseType == value)
					{
						found = TRUE;
			...
			default:
				if (value != SQL_C_DEFAULT)
				{
					for (i = 0, found = FALSE; gCDatatypeMap[i].conciseType != SQL_C_DEFAULT ; i++)
					{
						if (gCDatatypeMap[i].verboseType == value)
						{
							found = TRUE;
	...
	case SQL_DESC_DATETIME_INTERVAL_CODE:
		m_DescDatetimeIntervalCode = value;
	...
	case SQL_DESC_DATETIME_INTERVAL_PRECISION:
		m_DescDatetimeIntervalPrecision = (SQLINTEGER)LongValuePtr;
		break;
	case SQL_DESC_INDICATOR_PTR:
		...
		break;
	case SQL_DESC_LENGTH:
		m_DescLength = (SQLUINTEGER)ULongValuePtr;
		break;
	case SQL_DESC_NAME:
		strLen = m_DescDesc->m_ICUConv->FindStrLength((const char *)ValuePtr, BufferLength);
		...
		else
		{
			...
			m_DescDesc->m_ICUConv->InArgTranslationHelper((SQLCHAR*)ValuePtr, strLen, (char *)m_DescName,
																sizeof(m_DescName), &transLen, (char*)errorMsg);
			m_DescName[transLen] = '\0';
		}
		if (m_DescName[0] == 0)
			m_DescUnnamed = SQL_UNNAMED;
		else
	...
	case SQL_DESC_NUM_PREC_RADIX:
		m_DescNumPrecRadix = (SQLINTEGER)LongValuePtr;
		break;
	case SQL_DESC_OCTET_LENGTH:
		m_DescOctetLength = (SQLINTEGER)LongValuePtr;
		break;
	...
	case SQL_DESC_PARAMETER_TYPE:
		m_DescParameterType = value;
		break;
	case SQL_DESC_PRECISION:
		m_DescPrecision = value;
		break;
	case SQL_DESC_SCALE:
		m_DescScale = value;
		break;
	...
	CHandle(SQL_HANDLE_DESC, InputHandle)
{
	...
	m_ICUConv = m_ConnectHandle->m_ICUConv;
        m_replacementChar = m_ConnectHandle->getReplacementChar(); // 10-080310-1259
        m_ODBCAppVersion = m_ConnectHandle->getODBCAppVersion(); // 10-080310-1259
	m_DescRecCollect.reserve(500);
}
...
CDesc::~CDesc()
{
	...
	{
		if ( ((pDesc = (CDesc*)(*i)) != NULL) && (pDesc == this) )
		{
...
#else
	RWTPtrSlistIterator<CHandle> i(m_ConnectHandle->m_DescCollect);
	while ((pDesc = (CDesc*)i()) != NULL)
	{
		if (pDesc == this)
		{
			i.remove();
...
SQLRETURN CDesc::GetDescRec(SQLSMALLINT	RecNumber, 
					SQLCHAR *NameW,  //note that it could be UTF8 or UTF16
					...
					SQLSMALLINT	*NullablePtr)
{
	...
	if (NameW != NULL)
	{
		...
		else
		{
			if(strLen > 0)
			{
			//OutArgTranslationHelper(SQLCHAR* arg, int argLen, char * dest, int destLen, int *transLen, char *errorMsg)
				if(m_ConnectHandle->m_ICUConv->OutArgTranslationHelper((SQLCHAR*)descRecPtr->m_DescName, 
						strLen, (char*)NameW, BufferLength, (int*)&translateLength, NULL, (char*)errorMsg) == SQL_ERROR) 
...
//#endif
	...
	case SQL_DESC_BIND_TYPE:
		m_DescBindType = (SQLINTEGER)LongValuePtr;
		break;
	case SQL_DESC_COUNT:
		descValue = value;
		...
		else
		{
			...
			case SQL_DESC_MODE_UNKNOWN:
				...
				while (!m_DescRecCollect.empty() && descValue < m_DescCount)
				{
					...
					SQLINTEGER *StringLengthPtr)
{
	...
	case SQL_DESC_ARRAY_SIZE:
#ifdef __LP64__
		retValue.u.pValue = (SQLPOINTER)m_DescArraySize;
	...
	case SQL_DESC_COUNT:
//#ifdef __LP64__
//        retValue.u.pValue = (SQLPOINTER)m_DescCount;
//		retValue.dataType = SQL_IS_POINTER;
//#else
...
unsigned long CDesc::getDescRec(SQLUSMALLINT ColumnNumber, 
			SQLCHAR *ColumnName,  // locale/UTF8/UTF16
			SQLSMALLINT BufferLength, 
			...
			SWORD		errorMsgMax)
{
	...
	if (ColumnName != NULL)
	{
		...
		{
			if(m_ConnectHandle->m_ICUConv->isAppUTF16())
				(BufferLength % 2)? --BufferLength: BufferLength;
			if((retCode = m_ConnectHandle->m_ICUConv->OutArgTranslationHelper((SQLCHAR*)descRecPtr->m_DescName,
													strLen,
													(char*)ColumnName,
													BufferLength,
													(int*)&translateLength,
			...
			// ALM CR ID 5228. According to ODBC standard spec, the returned
			// described columname length shoudl be number of characters. Thus
			// in UTF16 case, in characters, it should be half of translateLength(in bytes).
			if(m_ConnectHandle->m_ICUConv->isAppUTF16())
...
//				memOffSet = ((memOffSet + 8 - 1) >> 3) << 3; 
				if(memOffSet % 2 == 1)  //Always get an even boundary
					SQLValue->dataValue._buffer = (IDL_octet*)(memPtr + memOffSet + 1);
				else
					SQLValue->dataValue._buffer = (IDL_octet*)(memPtr + memOffSet);
...
//			if((*ArrayStatusPtr) == SQL_ROW_IGNORE) break;
		}
					...
					ParamCount < ImpParamCount ; ParamCount++)
		{
			...
			{
				// CR ID 5228. When binding wide characters to UCS2 column, ConvertCToSQL incorrectly
				// converted get the length of the source string for returned DataType is SQL_C_CHAR.
				// Add IPDDescRecPtr->m_SQLCharset for determing the DataType.
				if ((retCode = getCDefault(IPDDescRecPtr->m_DescConciseType,ODBCAppVersion,IPDDescRecPtr->m_SQLCharset,DataType)) != SQL_SUCCESS)
				{
			...
			if (descRecPtr->m_DescOctetLengthPtr != NULL)
			{
				...
				{
					StrLenPtr = (SQLLEN*)((SQLULEN)descRecPtr->m_DescOctetLengthPtr + (RowCount * m_DescBindType));
				}
			...
			if (descRecPtr->m_DescIndicatorPtr != NULL)
			{
				if (m_DescBindType == SQL_BIND_BY_COLUMN)
					...
					IndPtr = (SQLLEN*)((SQLULEN)descRecPtr->m_DescIndicatorPtr + (RowCount * m_DescBindType));
			}
			else
				IndPtr = NULL;
			...
			if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
			{
				...
				if (IndPtr == NULL)
				{
					TraceOut(TR_ODBC_DEBUG, "CToSQL->inpData: <NULL> %s", "IndPtr");
				}
				else 
				{
					TraceOut(TR_ODBC_DEBUG, "CToSQL->inpData: IndPtr <%d>", *IndPtr);
				}

				if (DataPtr == NULL)
				{
					TraceOut(TR_ODBC_DEBUG, "CToSQL->inpData: <NULL> %s", "DataPtr");
				}
				else
				{
					if (StrLenPtr != NULL)
					...
					HexOut(TR_ODBC_DEBUG, Size, DataPtr, strcat(sTmp,"CToSQL->inpData"));
				}
			}
			...
			else
			{
				...
				{
					char tbyte;
					memcpy(&tbyte, SQLValue->dataValue._buffer, 1);
					memcpy((UCHAR*)SQLValue->dataValue._buffer, (UCHAR*)SQLValue->dataValue._buffer + 1 , 1);
					memcpy((UCHAR*)SQLValue->dataValue._buffer + 1,&tbyte, 1);
				}
#endif
				if(pHandle->getStmtQueryType() == SQL_INSERT_RWRS)
				{
					insertFormat = 1; //RWRS Insert format
					dateTimeIntervalPrecision = IPDDescRecPtr->m_DescDatetimeIntervalPrecision;
				}
				// 10-080310-1259
				retCode = ConvertCToSQL(ODBCAppVersion,
							...
							IPDDescRecPtr,
							byteSwap,
							m_ConnectHandle->getICUConverter(),
							errorMsg,
							sizeof(errorMsg),
                   ...
                   IPDDescRecPtr->m_SQLDataType != SQLTYPECODE_BOOLEAN)
				{
					...
					{
						if(IPDDescRecPtr->m_SQLCharset == SQLCHARSETCODE_UCS2)
				...
				if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
				{
					HexOut(TR_ODBC_DEBUG, SQLValue->dataValue._length, SQLValue->dataValue._buffer, "CToSQL->outData");
				}
		...
		if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
	{
		...
		if (IPD->m_DescArrayStatusPtr != NULL)
			HexOut(TR_ODBC_DEBUG, Size, IPD->m_DescArrayStatusPtr, "IPD->m_DescArrayStatusPtr");
	}
	return rc;
}
#else
		...
		SQLINTEGER	&ParamCharSet)
{
	...
	ParamCharSet = descRecPtr->m_ODBCCharset;
	return SQL_SUCCESS;
}
...
//			descRecPtr = m_DescRecCollect[ColumnCount];
			...
			if (DataPtr != NULL)
			{
				...
				else
				{
					...
					if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
					{
						HexOut(TR_ODBC_DEBUG, SQLDataLength, SQLDataValue, "SQLToC->inpData");
						if(StrLenPtr != NULL)
						   HexOut(TR_ODBC_DEBUG, *StrLenPtr, DataPtr, "SQLToC->outData");
					}
	...
	if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
	{
		long Size = sizeof(SQLUSMALLINT)* m_DescArraySize;
		if (IRD->m_DescArrayStatusPtr != NULL)
			HexOut(TR_ODBC_DEBUG, Size, IRD->m_DescArrayStatusPtr, "IRD->m_DescArrayStatusPtr");
		Size = sizeof(SQLUINTEGER);
		HexOut(TR_ODBC_DEBUG, Size, &RowsFetched, "RowsFetched");
	}
	if(CacheDescArray != NULL)
		delete[] CacheDescArray;
...
//	FPSQLDataSourceToDriver fpSQLDataSourceToDriver;
					...
					ColumnCount++)
		{
			...
			else
			{
				...
				if (SQLDataInd == 0)
				{
					...
					offset = offset + 1;
					SQLDataValue = SQLDataRow + offset;
					SQLDataLength = dataLengthFetchPerf(SQLDataType, SQLOctetLength, pStatement->getMaxLength(), (unsigned char*)SQLDataValue);
					offset = offset + SQLDataLength;
				}
				else
				{
	...
	if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
	{
		long Size = sizeof(SQLUSMALLINT)* m_DescArraySize;
		if (RowStatusArray != NULL)
			HexOut(TR_ODBC_DEBUG, Size, RowStatusArray, "RowStatusArray");
		Size = sizeof(SQLUINTEGER);
		HexOut(TR_ODBC_DEBUG, Size, &RowsFetched, "RowsFetched");
	}
	return rc;
			...
			BOOL		ColumnwiseData)
{
	...
	else
	{
        ...
        // 10-080310-1259 
		retCode = ConvertSQLToC(//m_ConnectHandle->getODBCAppVersion(),
					m_ODBCAppVersion,
					DataLang,
					SQLValue->dataType,
					descRecPtr->m_ODBCDataType,
					descRecPtr->m_SQLDatetimeCode,
					(char *)(SQLValue->dataValue._buffer),
		...
		if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
		{
			HexOut(TR_ODBC_DEBUG, SQLValue->dataValue._length, SQLValue->dataValue._buffer, "SQLToC->inpData");
			if(StrLen_or_IndPtr != NULL)
			   HexOut(TR_ODBC_DEBUG, *StrLen_or_IndPtr, TargetValuePtr, "SQLToC->outData");
		}
	}
							...
							SWORD		errorMsgMax)
{
	...
	if (DataType == SQL_C_DEFAULT)
	{
		if ((retCode = getCDefault(descRecPtr->m_DescConciseType,
								ODBCAppVersion, SQLCHARSETCODE_UNKNOWN, DataType)) != SQL_SUCCESS)
			return IDS_07_006;
	}
	...
	else
	{
		...
		if(pdwGlobalTraceVariable && *pdwGlobalTraceVariable)
		{
			...
			if (Size <= 0) Size = 16;
			HexOut(TR_ODBC_DEBUG, Size, DataAtExecData->dataValue._buffer, strcat(sTmp,"CToSQL->inpData"));
			HexOut(TR_ODBC_DEBUG, descRecPtr->m_SQLOctetLength, SQLValue->dataValue._buffer, "CToSQL->outData");
		}
...
SQLRETURN CDesc::SetPos(SQLSMALLINT NumResultCols)
{
	...
	for (int ColumnCount = 0 ; ColumnCount < NumResultCols ; ColumnCount++)
		//(getDescRecPtr (&m_DescRecCollect, ColumnCount))->m_DescReturnedLength = 0;
...
RWRS_Descriptor::RWRS_Descriptor(CDesc *ipd)
{
    int VarOffSet = 0;
    int memOffSet = 0;
    int IndOffSet = 0;
	...
	m_inputRowLength = 0;
    m_inputDescCount = ipd->getDescCount();

	m_inputColOffsets = new RWRS_columnOffsets[m_inputDescCount];

    for ( i = 0; i < m_inputDescCount; i++ )
    {
		inputDesc = ipd->m_DescRecCollect[i];

		if(inputDesc->m_DescNullable)
        {
            memOffSet = ((memOffSet + 2 - 1) >> 1) << 1;
            IndOffSet = memOffSet;     
            memOffSet += (int)sizeof(short) ;
            m_inputColOffsets[i].indLocation = IndOffSet;
        }
        else
            m_inputColOffsets[i].indLocation = -1;

        alignColumnOffsets (inputDesc->m_SQLDataType, inputDesc->m_SQLMaxLength, inputDesc->m_SQLDatetimeCode, &memOffSet, &VarOffSet );

        m_inputColOffsets[i].varLocation = VarOffSet;
    } 

    //The total rowlength should be aligned to the first column data type alignment.
	inputDesc = ipd->m_DescRecCollect[0];
    alignColumnOffsets ( inputDesc->m_SQLDataType, inputDesc->m_SQLMaxLength,inputDesc->m_SQLDatetimeCode, &memOffSet, &VarOffSet );

	m_inputRowLength = VarOffSet;
#ifdef DEBUG
    for ( i = 0; i < m_inputDescCount; i++ )
	printf ("Col %d Ind: %d  Var: %d \n", i, m_inputColOffsets[i].indLocation, m_inputColOffsets[i].varLocation);
    printf("row len: %d \n", m_inputRowLength);
#endif
}


void RWRS_Descriptor::alignColumnOffsets(SQLSMALLINT SQLDataType, SQLINTEGER SQLMaxLength, SQLINTEGER SQLDatetimeCode, int* MemOffSet, int* varOffSet )
{
    int VarOffSet = 0;
    int memOffSet = *MemOffSet;
